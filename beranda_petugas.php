...
if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] != 'petugas') {
...
$nama = isset($_SESSION['nama_lengkap']) ? $_SESSION['nama_lengkap'] : 'Petugas';

// Hitung jumlah data dari database
$barangCount = 0;
$penjualanCount = 0;

// Hitung jumlah barang dari tabel 'barang'
$queryBarang = mysqli_query($koneksi, "SELECT COUNT(*) AS total FROM barang");
if ($queryBarang) {
    $barangData = mysqli_fetch_assoc($queryBarang);
    $barangCount = $barangData['total'];
}

// Hitung jumlah penjualan dari tabel 'penjualan' (semua data)
$queryPenjualan = mysqli_query($koneksi, "SELECT COUNT(*) AS total FROM penjualan");
if ($queryPenjualan) {
    $penjualanData = mysqli_fetch_assoc($queryPenjualan);
    $penjualanCount = $penjualanData['total'];
}
?>
<!DOCTYPE html>
<html lang="id">
<head>
  ...
  <title>Dashboard Petugas | Kasir Ripan</title>
...
<body class="bg-gradient-to-br from-blue-100 to-white min-h-screen font-sans overflow-x-hidden">
    ...
    <div>
      <div class="flex items-center justify-between px-5 py-5 border-b border-blue-100 logo-container">
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-xl font-bold text-blue-700 logo-text select-none cursor-default">Petugas Panel</h2>
        </div>
      </div>

      <!-- MENU -->
      <nav class="mt-6 space-y-1">
        <a href="beranda_petugas.php" class="flex items-center gap-3 px-6 py-3 bg-blue-100 text-blue-700 font-semibold rounded-r-full transition">
  ...
  <main class="p-6 md:p-10 transition-all duration-300">
    <div class="bg-white rounded-3xl shadow-xl p-8 border border-blue-100">
      <h1 class="text-3xl font-bold text-blue-700 mb-3">Selamat Datang, <?= htmlspecialchars($nama); ?>!</h1>
      <p class="text-gray-600 text-lg">
        Anda sedang berada di <span class="font-semibold text-blue-600">Dashboard Petugas</span>.<br>
        Gunakan menu di sebelah kiri untuk mengelola data barang dan transaksi penjualan.
      </p>

      <!-- Kartu Statistik -->
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
        <div class="bg-gradient-to-br from-blue-400 to-blue-600 text-white rounded-2xl shadow-md p-6 flex flex-col items-center hover:scale-105 transition">
          <i class="fa-solid fa-box text-4xl mb-3"></i>
          <h3 class="text-lg font-semibold">Total Barang</h3>
          <p class="text-sm opacity-90 mt-1"><?= $barangCount; ?> Item</p>
        </div>

        <div class="bg-gradient-to-br from-blue-500 to-blue-700 text-white rounded-2xl shadow-md p-6 flex flex-col items-center hover:scale-105 transition">
          <i class="fa-solid fa-cart-shopping text-4xl mb-3"></i>
          <h3 class="text-lg font-semibold">Total Penjualan</h3>
          <p class="text-sm opacity-90 mt-1"><?= $penjualanCount; ?> Transaksi</p>
        </div>

        <div class="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-2xl shadow-md p-6 flex flex-col items-center hover:scale-105 transition">
          <i class="fa-solid fa-user text-4xl mb-3"></i>
          <h3 class="text-lg font-semibold">Petugas Aktif</h3>
          <p class="text-sm opacity-90 mt-1"><?= htmlspecialchars($nama); ?></p>
        </div>
      </div>
      ...
      <div class="mt-10">
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-2xl p-6 shadow-sm">
          <p class="text-gray-600">Anda terakhir login sebagai <b><?= htmlspecialchars($_SESSION['user_role']); ?></b> pada <?= date("d M Y, H:i"); ?>.</p>
          <p class="mt-2 text-sm text-gray-500">Tetap semangat bekerja hari ini! 💪</p>
        </div>
      </div>
    </div>
  </main>
